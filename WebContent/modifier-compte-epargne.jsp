...
<title>Page de modification du compte épargne</title>
...
<body>
	...
	<%--
	 ...
	 --%>
	<%
		CompteEpargne ceUpdate = (CompteEpargne)request.getAttribute("compte_modif");
	%>
	...
	<form method="POST" action="gestion-compte-servlet">
		...
		<table>
			<tr>
				...
				<td><input type="text" value="<%= ceUpdate.getSoldeCompte() %>" name="p-solde"/> </td>
			</tr>
			<tr>
				<td>Découvert Autorisé :</td>
				<td><input type="text" value="<%=ceUpdate.getTauxInterets()%>" name = "p-taux" /> </td>
			</tr>
			<tr>
				<td>Reférence Client :</td>
				<td><input type="text" value="<%= ceUpdate.getIdTitulaire() %>" name = "p-titulaire" /> </td>
			</tr>			
			...
			<tr>
				<td><input type="hidden" value="<%= ceUpdate.getNumeroCompte() %>" name="p-numCompte"/></td>
			</tr>
			<%-- passage du param action dans la requete. sa valeur est 'modification' --%>
			<tr>
				<td><input type="hidden" value="modification-compte-epargne" name="action"/></td>
			</tr>			
